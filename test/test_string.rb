...
module Coaster
  class TestString < Minitest::Test
    def test_string
      ...
      with_symbol = true
      # half -> full 1:1 잘 변환되는가?
      assert_equal half_seq.to_full_characters(with_symbol:), full_seq
      # half -> full -> half 변환 시, 원래 string 유지되는가?
      assert_equal half_seq.to_full_characters(with_symbol:).to_half_characters(with_symbol:), half_seq
      # not_target string은 to full 변환 시도시 원본이 유지되는가?
      assert_equal not_target_half_seq.to_full_characters(with_symbol:), not_target_half_seq
      # target / not_target이 섞여있는 문장에서, target'만' full로 변환되는가?
      assert_equal mixed_str.to_full_characters(with_symbol:), full_seq + not_target_half_seq
      # target / not_target이 섞여있는 문장에서, half -> full -> half 변환시, 원래 string 유지되는가?
      assert_equal mixed_str.to_full_characters(with_symbol:).to_half_characters(with_symbol:), mixed_str
      # 공백문자열에 다른게 추가되지는 않는가?
      assert_equal ''.to_full_characters, ''
      ...
      with_symbol = true
      # full -> half 1:1 잘 변환되는가?
      expect(full_seq.to_half_characters(with_symbol:)).to eq half_seq
      assert_equal full_seq.to_half_characters(with_symbol:), half_seq
      # full -> half -> full 변환 시, 원래 string 유지되는가?
      expect(full_seq.to_half_characters(with_symbol:).to_full_characters(with_symbol:)).to eq full_seq
      assert_equal full_seq.to_half_characters(with_symbol:).to_full_characters(with_symbol:), full_seq
      # not_target string은 to half 변환 시도시 원본이 유지되는가?
      expect(not_target_full_seq.to_half_characters(with_symbol:)).to eq not_target_full_seq
      assert_equal not_target_full_seq.to_half_characters(with_symbol:), not_target_full_seq
      # target / not_target이 섞여있는 문장에서, target'만' half로 변환되는가?
      expect(mixed_str.to_half_characters(with_symbol:)).to eq half_seq + not_target_full_seq
      assert_equal mixed_str.to_half_characters(with_symbol:), half_seq + not_target_full_seq
      # target / not_target이 섞여있는 문장에서, full -> half -> full 변환시, 원래 string 유지되는가?
      expect(mixed_str.to_half_characters(with_symbol:).to_full_characters(with_symbol:)).to eq mixed_str
      assert_equal mixed_str.to_half_characters(with_symbol:).to_full_characters(with_symbol:), mixed_str
      # 공백문자열에 다른게 추가되지는 않는가?
      expect(''.to_half_characters(with_symbol:)).to eq ''
      assert_equal ''.to_half_characters(with_symbol:), ''
